...
public class ManageUsersFrame extends JFrame implements ActionListener {
    ...
    private final Dimension labelDimension = new Dimension(100, 20), inputBoxDimension = new Dimension(140, 20),
    ...
    ManageUsersFrame() throws SQLException{
        ...
        userAccessLevelLabel.setForeground(Color.WHITE);
        ...
        userAccessLevelComboBox.setForeground(Color.WHITE);
        userAccessLevelComboBox.setBackground(Color.GRAY);
        ...
        userDataTable.setForeground(inputColor);
        userDataTable.setBackground(mainColor);
        userDataTable.setSelectionBackground(Color.GREEN);
        userDataTable.setSelectionForeground(mainColor);
    ...
    private void dbUpdateUser(User user) {
        if (isBoxesEmpty()) {
            JOptionPane.showMessageDialog(null, "You must fill all text fields!",
                    "Input error", JOptionPane.WARNING_MESSAGE);
        }else if (userDataTable.getSelectionModel().isSelectionEmpty()) {
            JOptionPane.showMessageDialog(null, "You must pick a line from the table!",
                    "Update error", JOptionPane.WARNING_MESSAGE);
        }else {
            try {
                user.setUsername(userDataTable.getValueAt(userDataTable.getSelectedRow(),0).toString());
